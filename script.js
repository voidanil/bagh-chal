...
var position = document.getElementsByClassName("position"); //stores the collection of  all 25 positions in the board
...
var tigerTrappedCount;              //counts the number of tiger trapped
var winningMessage = "";            //winner message displayed

var movesValidate = [[1, 5, 6], [0, 2, 6], [1, 3, 6, 7, 8], [2, 4, 8], [3, 8, 9],
...
[15, 16, 21], [16, 20, 22], [16, 17, 18, 21, 23], [18, 22, 24], [18, 19, 23]]; //check the possible normal moves position 

var tigerKillsMovesVal = [[2, 10, 12], [, 3, 11], [0, 4, 10, 12, 14], [1, , 13], [2, 12, 14],
[, 7, 15], [, , , , 8, , 16, 18], [, 5, 9, 17], [, , , 6, , 16, 18,], [, 7, 19],
[0, 2, 12, 20, 22], [1, , 13, 21], [0, 2, 4, 10, 14, 20, 22, 24], [3, 11, , 23], [2, 4, 12, 22, 24],
[5, 17,], [, 6, 8, , 18, , ,], [7, 15, 19,], [6, 8, , 16, , , ,], [9, 17,],
[10, 12, 22], [11, , 23], [10, 12, 14, 20, 24], [13, 21,], [12, 14, 22]];//check the possible moves for Tiger kill
...
function tigerKills(index) {
    const killedGoatPos = movesValidate[presviousPos][tigerKillsMovesVal[presviousPos].indexOf(index)];
    if (positionArray[killedGoatPos].hasChildNodes()) {
        const killedGoatCheck = positionArray[killedGoatPos].firstChild.classList.contains('goat');
        if (killedGoatCheck) {
...
function tigerTrappedCheck() {
    ...
    tigerTrappedCount != 0 ? console.log("Tiger trapped = " + tigerTrappedCount) : "";  //just to console trapped tiger count
}
...
document.getElementById("restart").onclick = () => {
    ...
    positionArray.forEach((pos) => {        //removes all pieces from board
    ...
    pos.addEventListener("click", (e) => {
        ...
        if (turn % 2 == 1 && bingo != true) {
            ...
            if (pos.hasChildNodes() && turn > 40 && turn % 2 == 1) {
                ...
                if (checkClass) {
                    ...
                    if (presviousPos != -1) {
                        positionArray[presviousPos].firstChild.classList.remove('selectedPiece');
                    }
                    presviousPos = index;
            ...
            if (!pos.hasChildNodes() && turn > 40 && turn % 2 == 1 && presviousPos != -1 && validateMoves(index)) {
                imgSelect(img);
                positionArray[presviousPos].removeChild(positionArray[presviousPos].firstChild);
                pos.appendChild(img);
                presviousPos = -1;
                bingoCheck();
                turn++;
        ...
        if (turn % 2 == 0 && bingo != true) {
            ...
            if (!pos.hasChildNodes() && presviousPos != -1 && validateMoves(index)) {
                ...
                pos.appendChild(img);
                if (killGoat) {
                    positionArray[killGoatPos].removeChild(positionArray[killGoatPos].firstChild);
                    killGoat = false;
